...
# Grafana Cloud — CloudWatch cross-account integration (see grafana_iam.tf)
# Inject External ID at runtime only: export TF_VAR_grafana_cloud_external_id="..."
# Never commit External ID to .tfvars or source control.
variable "grafana_cloud_aws_account_id" {
  description = "The AWS Account ID owned by Grafana Cloud"
  type        = string
  # Grafana Labs shared AWS account for CloudWatch integrations (Grafana Cloud Free Tier)
  default     = "008923505280"
}

variable "grafana_cloud_external_id" {
  description = "The unique External ID generated by Grafana Cloud"
  type        = string
  sensitive   = true
}
